...
diabetes <- read.csv(file="./kdd project/diabetes.csv",stringsAsFactors = FALSE)
diabetes
...
# summary(diabetes$Pregnancies)
 ...
 completedData_normalized<-completedData_training[c(10,12:17)]
 ...
 #find the number of no and yes values in the outcome column
 ...
 cvCont <- trainControl(method = "repeatedcv", number = 5,
                        ...
                        classProbs = TRUE)
 install.packages("caret",lib = "/Downloads/caret-master")
 library(caret)
 install.packages("C50", lib = "/Downloads/C50_0.1.0-24")
 library(C50)
 install.packages("pROC",lib = "/Downloads/pROC")
 library(pROC)
 ...
 set.seed(1)
 install.packages("plyr",lib = "/Downloads/plyr")
 library(plyr)
 ...
 install.packages("ROCR", lib = "/Downloads/ROCR_1.0-5")
 library(ROCR)
 p <- predict(model, newdata=subset(completedData_test,select=c(1,2,3,4,5,6,7,8,10,11,12,13,14)), type="response")
 pr <- prediction(p, completedData_test$Outcome)
